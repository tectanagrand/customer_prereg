...
const INCOTERM = [
    { value: "FRC", label: "FRANCO" },
    { value: "LCO", label: "LOCO" },
];

export default function MultiLNWB() {
    ...
    const CGRP = useMemo(() => loaderdata.C_GRP, [loaderdata]);
    const {
        control,
        reset,
        handleSubmit,
    ...
    const SubmitForm = async values => {
        let payload = {
            ...
            ven_code: values.transporter?.value ?? "",
            ven_name: values.transporter?.label.split("-")[0].trim() ?? "",
    ...
    useEffect(() => {
        if (!loading && data && searchParams.get("id")) {
            reset({
                ...data,
    ...
    useEffect(() => {
        if (onLoad && getFieldState("incoterm").isDirty) {
            setValue("requests", []);
            setIsPaid(false);
        }
    }, [watch("incoterm")]);
    ...
    return (
        <>
            <Toaster />
            <h2>Multi Loading Note Request Form</h2>
            <br />
            <form
