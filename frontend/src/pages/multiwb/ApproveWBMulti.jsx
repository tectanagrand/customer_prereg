...
export default function PushWBMulti() {
    ...
    const [urlFetchData, setUrlFetch] = useState("");
    const [value_cust, setValueCust] = useState(null);
    ...
    const columns = useMemo(
        () => [
            ...
            columnHelper.accessor("cust_code", {
                header: "Customer",
                cell: ({ row }) =>
                    `${row.original.cust_name} (${row.original.cust_code})`,
            }),
            ...
            columnHelper.accessor("ven_code", {
                header: "Transporter",
                cell: ({ row }) =>
                    `${row.original.ven_name} - ${row.original.ven_code}`,
            }),
    ...
    const {
        data: cust_op,
        loading: loading_custop,
        error: error_custop,
    } = useFetchData({
        url: "/multi/listcustos?prereg_type=WB",
        initData: {
            data: [],
        },
    });

    const op_cust = useMemo(() => {
        return cust_op.data
            ? cust_op.data.map(data => ({
                  value: data.cust_code,
                  label: `${data.cust_name} (${data.cust_code})`,
              }))
            : [];
    }, [cust_op]);
    ...
    const FetchFunction = debounce(cust => {
        if (cust) {
            const cust_code = cust.value;
            setUrlFetch("/multi/osreq?prereg_type=WB&cust=" + cust_code);
            refreshTable();
        } else {
            setUrlFetch("");
            refreshTable();
...
function SubmissionSelectedModal({
    ...
    useEffect(() => {
        ...
        //tells user sloc and valtype need to be maintained
        if (!allow) {
            setIsAllowed(allow);
