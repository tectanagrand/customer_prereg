...
const TableChildMultiWB = ({ dataChild }) => {
...
export default function DashboardMultiLNWB() {
    const navigate = useNavigate();
    ...
    useEffect(() => {
        (async () => {
            if (refresh) {
                try {
                    const { data } = await axiosPrivate(`/multi/wb/requests`);
                    setData({
